...
builder.Register(x => new EfContextUnitOfWorkAdapter(conString)).As<IUnitOfWork>().SingleInstance();
...
foreach (var item in unitOfWork.FoodRepo.Query().OfType<Drink>().Where(x => !x.Alc).OrderByDescending(x => x.KCal).ToList())
{
